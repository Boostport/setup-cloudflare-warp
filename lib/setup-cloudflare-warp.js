...
async function checkWARPRegistration(organization, is_registered) {
  const maxRetries = 6; // 30 seconds / 5 seconds
  let retries = 0;

  let output = "";

  while (retries < maxRetries) {
    const result = await exec.getExecOutput("warp-cli", ["--accept-tos", "settings"]);
    const stdout = result.stdout;
    const stderr = result.stderr;

    output = stdout;

    const registered = stdout.includes(`Organization: ${organization}`) || stderr.includes(`Organization: ${organization}`);

    if (is_registered && registered) {
      return; // Registered as expected
    } else if (!is_registered && !registered) {
      return; // Unregistered as expected
    }
...
async function checkWARPConnected() {
  const maxRetries = 15; // 30 seconds / 2 seconds
  let retries = 0;

  while (retries < maxRetries) {
    const statusExecOutput = await exec.getExecOutput("warp-cli", ["--accept-tos", "status"]);
    const stdout = statusExecOutput.stdout;
    const stderr = statusExecOutput.stderr;

    // Retry connect on missing registration
    if (stdout.includes("Reason: Registration Missing") || stderr.includes("Reason: Registration Missing")) {
      await exec.exec("warp-cli", ["--accept-tos", "connect"]);
    }

    if (stdout.includes("Status update: Connected") || stderr.includes("Status update: Connected")) {
      return; // Connected!
    }
  ...
  throw new Error("WARP is not connected after 30 seconds");
}
...
export async function run() {
  ...
  await checkWARPRegistration(organization, true);
  await exec.exec("warp-cli", ["--accept-tos", "connect"]);
