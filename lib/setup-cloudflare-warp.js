...
async function installLinuxClient(version) {
  ...
  // Special case ubuntu lunar to use bookworm instead
  await exec.exec(
    "/bin/bash -c \"if [ $(lsb_release -cs) = 'lunar' ]; then sudo sed -i 's/lunar/bookworm/g' /etc/apt/sources.list.d/cloudflare-client.list; fi\"",
  );
