...
async function installLinuxDeb(version) {
  ...
  const distroCodeName = (
    await exec.getExecOutput("lsb_release", ["-c"])
  ).stdout
    .split(":")[1]
    .trim();
  ...
  await exec.exec(
    `bash -c "echo 'set man-db/auto-update false' | sudo debconf-communicate"`,
  );
  await exec.exec("sudo rm -f /var/lib/man-db/auto-update");

  await exec.exec("sudo apt-get -o DPkg::Lock::Timeout=-1 update");
  await exec.exec(
    `sudo apt-get -o DPkg::Lock::Timeout=-1 install -y --no-install-recommends ./${debPath}`,
  );
}
...
async function installLinuxClient(version) {
  ...
  await exec.exec("sudo apt-get -o DPkg::Lock::Timeout=-1 update");
  await exec.exec(
    "sudo apt-get -o DPkg::Lock::Timeout=-1 install -y cloudflare-warp",
  );
}
